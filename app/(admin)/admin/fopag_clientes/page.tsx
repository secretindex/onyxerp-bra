...
function EstadoSelect() {
  ...
  const handleAddFopagClient = () => {
    axios.post("/api/fopag", {
      estado: stateUf,
      ...
      .then((res) => {
        ...
        toast.success("Cliente de FOPAG adicionado com sucesso");

        setTimeout(() => {
          redirect("/admin/fopag_clientes");
        }, 1000);
      })
      .catch((err) => toast.error("Erro ao adicionar cliente de FOPAG", err));
  }
  ...
  return (
    <section className="flex flex-col gap-2 w-full bg-card">
      ...
      <Field className="w-full">
        ...
        <Input type="text" placeholder="Adicione o link de acesso" value={link} onChange={(e) => setLink(e.target.value)} />
      </Field>
      <Field className="w-full">
        <Button onClick={handleAddFopagClient} className="w-full">Adicionar</Button>
      </Field>
    </section>
  );
}

const FopagClientesPage = () => {
  return (
    <section className="min-h-screen flex flex-col justify-center w-full items-center px-4 py-8">
      <div className="flex flex-col gap-4 w-full sm:w-4/5 md:w-3/5 lg:w-2/5 max-w-xl bg-card p-6 border border-border shadow-lg rounded-md">
        ...
        <div className="flex flex-col gap-2">
          <h1 className="text-2xl font-bold">Clientes de FOPAG</h1>
          <p className="text-xs">
            Adicione um novo cliente de FOPAG
          </p>
        </div>
...
export default FopagClientesPage;
